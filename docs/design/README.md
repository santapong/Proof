...
Two machine-readable artifacts that govern v1.0.0. They are **normative**, not historical notes.

| File | What it is | Authority |
|---|---|---|
| `boundary-audit.json` | The 18-skill scope matrix: one mutually-exclusive scope line per skill, every rated overlap with its resolution, and the approved `description:` text for each skill. | **Outranks the build manifest.** Where a plan and this file disagree, this file wins. |
...
Skill selection happens on the `description:` field alone, before any skill body is read. With eighteen skills, the descriptions have to be mutually exclusive by construction, and the reasoning for *why* a boundary sits where it does has to be as durable as the boundary itself.
