# C4 System Context (Level 1) — Proof

**What is this system, who uses it, and what does it depend on?**

Proof is a **Claude Code plugin**: twenty-six composable engineering skills built on a multi-agent workflow engine. It ships skill instructions, workflow templates, and the local `proof-mcp` server. The Claude Code host supplies workflow execution and agent runtime capabilities. That is the single most important thing this diagram communicates, and it is why Claude Code appears *outside* the box rather than inside it.

![C4 System Context — Proof](diagrams/context.svg)
...
- **Claude Code** discovers the plugin, loads the skill into context, and executes the workflow scripts the skill authors — Proof never runs anything itself.
...
No skills, no policies, no templates, no `ROUTES` block, no phases. Twenty-six skills drawn as twenty-six peer boxes would be the classic Level-1 mistake: from the outside, this is one plugin with one entry point per task. Open the box at [Level 2 — Container](container.md).
