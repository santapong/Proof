...
**Say what it does, when to use it, and which sibling to use instead.** The third clause is what makes nineteen descriptions mutually exclusive rather than merely different. Every rated overlap in [`boundary-audit.json`](../design/boundary-audit.json) is resolved by a "use X instead when Y" pointer on *both* sides — a one-way pointer leaves the boundary decidable from only one direction, which is a defect the audit checks for.
