...
### Changed
- **README rewritten for the front page it is.** The 350-line README — whose header still said "22 composable skills" and "eighteen domain skills" two releases on — is now a ~100-line professional front page: install first, the where-to-start table, one compact 24-skill group table, the composition diagram, four how-it-works bullets, host portability, contributing. Everything it dropped moved intact to the new **`docs/overview.md`** (the full tour: Why, the detailed per-skill tables, the autonomy ladder with the SCALE rationale, the engine walkthrough, the execution-mode table, branch-per-task and the integration train, the complete repository layout) — a README section is now a summary with a link, not the only home of the prose.
- **The skill-composition diagram redesigned, and every diagram source de-staled to the 24-skill fleet.** `skill-composition.mmd` was two releases behind (twenty-two in the title, `loop-v1` still on the canvas, `loop-context`/`loop-comprehend`/`loop-guide` missing) and visually flat — one pale blue for every node, dashed outlines for every group. Rebuilt: all 24 skills, role groups carrying family tints over washed backgrounds (engine blue, knowledge violet, design/verify green, ship/run red, automation teal), shape as a second channel — stadium front door, hexagon engine, double-edged law, rounded skills — the operational cycle folded horizontally, and `loop-guide` drawn where it sits: above the groups it routes into. The grammar extension is documented in `docs/c4/README.md` next to the existing fill table. Also de-staled: `skills-engine-planning.mmd` (`loop-v1` → `loop-build`, + `loop-context`/`loop-guide`, the 21-skill matrix label), `skills-knowledge-automation.mmd` (+ `loop-comprehend` with its two handoffs), `view-logical.mmd` (6 + 17 + 1 grouping), `context.mmd`/`container.mmd`/`view-scenarios.mmd` counts (24 skills, 121 references, 29 templates, 20-of-24 packs). The same palette then swept the whole directory: every diagram source moved off the flat C4 blues onto the family palette (light fills with dark text for containers and skills, saturated blue reserved for the engine and `heimdall-mcp`, warm-gray externals, amber gates/notes), every dashed-gray group outline became a washed family tint, the four category diagrams carry their family colors, the autonomy ladder's rungs deepen through the teal family with SCALE dashed (off by default), and the process sequence diagram's actors/notes joined the theme. All 18 diagrams re-rendered.
...
- **Track 2 / H2 — Tier C discovery.** [`docs/design/host-tier-c-discovery.md`](docs/design/host-tier-c-discovery.md): as of Aug 2026 all three target hosts have programmable multi-agent surfaces (Cursor SDK subagents, Codex Multi-Agent v2 TOML roles, Antigravity 2.0 squads) and none executes Heimdall's workflow contract. Per-host verdicts: Cursor is the only plausible adapter target; Codex expresses static fan-out/consolidate only (the unearned barrier, as a platform); Antigravity is a redesign against a ten-week-old surface. Tier C stays Claude-Code-only, stated as a fact about the templates, not the hosts; ADR-0010's answer is "not now, and never per-host." Every claim is graded secondary-source and re-confirm-before-implementing — nothing was verified against a running host.
- **ADR-0008 accepted (2026-08-04)** — the ruling H0 deferred: the seam held through H1 (all 32 pointers closed under §D8.9's carry/stub mechanism, the check promoted to a failing gate) and absorbed two new skills with no mechanism change — packs now carry **20 of 24 skills, 126 files each**. ROADMAP, INSTALL and the design index updated from 18-of-22; the Track 2 status line now says which halves await a human at a host window (H1/H3/H4 session gates).

### Changed
- **Track 1 (Rust runtime for `heimdall-mcp`) removed from `ROADMAP.md` — it never existed.** The roadmap had opened with two tracks; only host portability is real. The Rust rewrite was never a plan, so its track, its ADR-0009 slot, and its R0–R4 phase table are removed rather than left standing as a promise nobody made; the three places where the binary was load-bearing now name `node` on `PATH` as the standing requirement.

### Fixed
- **`loop-pattern/references/design-patterns.md` shipped its scaffolding** — a six-line "Drafting notes for the caller" block (with a stale `/mnt/data/company/…` path) survived from the v2.0.0 deepening workflow into the released file; H1's residue report caught it as the one source defect among the packing warnings. Removed.

## [2.2.0] — 2026-08-04

### Added
- **Track 2 / H0 — the host-packaging seam.** [`docs/design/ADR-0008-host-packaging-seam.md`](docs/design/ADR-0008-host-packaging-seam.md) (**Proposed**, needs a ruling) plus the tooling it specifies: `scripts/host-targets.json` (data-only descriptors for Cursor, OpenAI Codex and Antigravity — skills paths, MCP config format, the held-back list, the closed rewrite-rule list, each rule carrying the clause that authorises it), `scripts/pack-host.mjs` (generates `dist/<host>/`, Node stdlib only), `scripts/check-host-packs.mjs` (the D8.8 gate, wired into `.github/workflows/validate.yml`), and a first `.gitignore` covering `dist/`. Three findings drive the ADR, all measured on this tree: **four skills are host-native by subject, not by accident** — `loop-engine`, `loop-harness`, `loop-skill`, `loop-autopilot` — so a pack carries **18 of 22 skills**, not 22; **generated trees must not live at the paths hosts auto-discover**, because Cursor also reads `.claude/skills/`, `.codex/skills/` and `.agents/skills/`, and a Cursor session in this checkout would otherwise load every skill three times; and the **509 Claude-model-name references are not the obstacle they look like** — all but two prose files sit inside the 28 `*.workflow.js` templates, which no non-Claude pack ships. Nothing in `.claude/skills/` changed: the source tree is untouched and remains the only place a skill is edited. **Gate run on this tree (Node 22):** three packs, 18 skills and 111 files each, deterministic, no forbidden token, no template survivor, every degraded skill carrying its banner — green. Its residue report then found 32 dangling cross-skill pointers per host in three classes (the autonomy ladder's definitional home now sits in a held-back skill; 13 mode-dial pointers; `loop-orchestrate`'s 16 references into `loop-engine`), recorded as open questions on the ADR rather than fixed silently, because each is a decision. They block publishing a pack, not the seam.
- **`ROADMAP.md` — two proposed tracks, neither decided.** Track 1: a **Rust runtime for `heimdall-mcp`**, one static binary per platform, justified by distribution rather than speed and gated on a new ADR that has to amend or supersede [ADR-0001](mcp/ADR-0001-runtime-and-dependency.md) (Node stdlib, no manifest, no dependency) and re-argue the no-manifest law in `CONTRIBUTING.md:69` / `scripts/validate.mjs:12`. Track 2: **host portability** beyond Claude Code, first three targets **Cursor, OpenAI Codex, Antigravity**, with a per-host support tier (A skills load / B MCP tools reachable / C multi-agent execution) and a generated-per-host packaging seam so `.claude/skills/` stays the single source of truth and host #4 costs a checklist. The roadmap records what does *not* port — the 28 `*.workflow.js` templates target Claude Code's `Workflow` tool and no target host has a known equivalent, which is an open discovery task, not a promise. Host config paths in the target matrix are dated 2 Aug 2026 and marked re-confirm-before-implementing. README gains a Roadmap section, a `mcp/` row in the repository layout (previously missing), and a contents entry.
- **`loop-guide`** — the twenty-fourth skill: the front door. For the developer who does not know which of the twenty-four skills fits: a minimum-discriminating interview (mine the request first, every question must eliminate a candidate, one to three questions, batched), classification on the boundary audit's checkable questions phrased in user terms, then a routing verdict — single skill, ordered chain, or escalation to `loop-build`/`loop-orchestrate`/`loop-skill` — with a **because-line** on every pick (the one-sentence checkable answer that selected it, stated before work runs, so the user can refute a misroute in seconds). Dispatch fills the routed skill's arguments from the interview so nothing is re-asked; chains are conducted between hops with re-validation (the route is a hypothesis, hops are evidence; live user impact re-routes to `loop-incident` mid-chain) and a ~5-hop tripwire escalating to `loop-build`. Deliberately ships **no workflow template and no `--mode`** — interviewing needs a human, which the sandbox does not have (the `loop-harness` precedent). The routing instrument is `boundary-audit.json`, read at routing time and never restated — the duplicate-instrument trap is #5 in its own failure catalogue (`guide-traps.md`: the guide that does the work, interview bloat, routing by topic word, the chain that should be loop-build, the duplicate instrument, the confident misroute, the sticky route). Boundary registered as three audit entries with reciprocal pointers added to `loop-orchestrate`, `loop-build`, and `loop-skill`; the shelf pins Snowden & Boone (HBR, Nov 2007) for classify-before-respond, the Claude Code skills docs as the selection mechanism being compensated for, and lists the audit itself flagged as internal governance, not a standard. Counts refreshed to twenty-four.
- **`loop-comprehend`** — the twenty-third skill: understand code that already exists. Architecture recovery via extract → abstract → present (Ducasse & Pollet's taxonomy) with hypothesis-driven reading and one-grep boundary checks; feature tracing with both-ends anchoring and an indirection-resolution table; decision recovery — reverse-engineered ADRs for decisions the record never captured, with a mandatory evidence/inference split and per-ADR confidence grades (`evidenced` / `inferred` / `speculative`); the onboarding dossier as the composite deliverable, stamped with the commit it describes. Ships born with its failure catalogue (`comprehension-traps.md`: the README map, directory-equals-component, confident recovered history, single-path generalization, the unverified operational quote, coverage silence, comprehension creep, the stale dossier). Non-negotiable: a claim about a codebase carries a `file:line` or it is a guess, and every deliverable states its coverage. Boundary registered as five audit entries with reciprocal pointers added to `loop-design`, `loop-docs`, and `loop-audit`; shelf pins ISO/IEC/IEEE 42010:2022 (the one Yes-grade), TSE 35(4) 2009, Naur 1985, Feathers 2004, Nygard 2011, MADR 4.0.0, C4, Views and Beyond 2e — confirmation log distinguishes the three verified this pass from the five carried. Template: `comprehend.workflow.js` (inventory → per-dimension map → per-claim evidence verify → dossier synthesis). Skill counts refreshed to twenty-three across README, INSTALL, and the C4 docs (INSTALL was still saying twenty-one — stale since v1.5.0).
