// mcp/server.mjs — proof-mcp, the stdio JSON-RPC shell (Phase 3, S7).
//
...
// file would need. PROOF_ROOT overrides it (THELOOPSKILL_ROOT accepted as a deprecated alias) (the packaged-install escape hatch); none of the
// mcp/lib/*.mjs files read that env var themselves (verified: no PROOF_ROOT or THELOOPSKILL_ROOT reference in any
...
// operator's PROOF_ROOT override. That would be exactly the silent default ADR-0005
...
// PROOF_ROOT is the name; THELOOPSKILL_ROOT is accepted one major as a deprecated
// alias (renamed with proof-mcp 0.2.0) so existing wiring keeps booting — but it says
...
const ROOT = process.env.PROOF_ROOT
  ? path.resolve(process.env.PROOF_ROOT)
  : LEGACY_ROOT ? path.resolve(LEGACY_ROOT) : DEFAULT_ROOT

const SERVER_NAME = 'proof-mcp'
...
// "proof-mcp: " (ADR-0002 C3) so an operator scanning Claude Code's own stderr can grep it
...
function stderrLine(text) {
  process.stderr.write(`proof-mcp: ${text}\n`)
}

if (!process.env.PROOF_ROOT && LEGACY_ROOT) {
  stderrLine('THELOOPSKILL_ROOT is deprecated (server renamed proof-mcp in 0.2.0) — rename the variable to PROOF_ROOT; the alias will be dropped in the next major')
}
...
function sourceDocsMissingMessage(what) {
  return `proof-mcp: source docs not found — ${what} under ${ROOT} · fix: set PROOF_ROOT to a Proof checkout, or correct the server path in .mcp.json`
}
function nodeTooOldMessage() {
  return `proof-mcp: node 18 or newer required — running ${process.version} · fix: upgrade node, or point .mcp.json "command" at a newer binary`
}
...
function runDocShapeProbes() {
  ...
  for (const probe of probes) {
    ...
    if (!result || result.ok !== true) {
      ...
      stderrLine(
        `boot doc-shape probe failed — ${probe.label}: ${detail} · fix: set PROOF_ROOT to a Proof checkout, or correct the server path in .mcp.json ` +
...
const CITATION_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    resourceUri: { type: 'string', description: "proof:// URI of the same file, readable via this server's resources/read." },
  },
})
