...
Context pressure, lossy compaction, and stale facts are separate failure modes. The research in `references/standards.md` motivates testing them separately; its context-size figures describe particular benchmarks, not a validated budget for every model or software task.
...
**For reducing software-team context cost**, read `references/software-efficiency.md`: retrieve the smallest relevant packet, preserve evidence pointers, and compare actual usage and delivery outcomes. Static prompt-size estimates are proxies, not billed-token measurements. Local classification is optional advice and does not change execution routing or verification requirements.
...
- **Set an explicit context budget for the task and model.** Treat the reference's 10–20% band as a benchmark-derived starting hypothesis, then adjust using representative answer-preservation checks and measured usage. The vendor window is an input ceiling, not evidence of effective reasoning at that size.
...
- `references/software-efficiency.md` — selective software context, recoverable handoffs, optional local ML, and usage-versus-quality measurements
