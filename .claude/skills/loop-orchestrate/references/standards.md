...
- **Authoritative — yes.** A recognized standards body or licensed framework owner **ratified and published** it. On this shelf: the **PMI standards** (PMBOK Guide, *Practice Standard for WBS*) and **SAFe** as a licensed framework. These carry weight on their own — "the WBS 100% Rule" is a rule with an owner.
- **Authoritative — draft.** Real, citable committee output that **nothing has ratified**. *No entry on this shelf carries this grade.*
- **Authoritative — no.** Everything else here: **RICE** (one company's scoring model), **MoSCoW** as popularly used, the **Kano model** (a 1984 research model), and **CPM / PERT / Amdahl / Brooks** — theorems, methods, and empirical results, which are stronger than opinion and are still not anybody's specification.

The practical consequence: an ordering justified by WSJF or the 100% Rule can be defended as *the named method's* answer. An ordering justified by RICE is defensible only if you show the inputs, because nobody ratified the weights.
...
|---|---|---|---|---|
| **WSJF** (Weighted Shortest Job First) | Scaled Agile, Inc. (**SAFe**); originally Reinertsen, *Product Development Flow* | **SAFe 6.0** (Mar 2023) — the edition the CoD axes below were mapped against, **not the newest**. Scaled Agile released **AI-Native SAFe on 23 June 2026** as a new version of the framework and is **dropping numbered versions** in favour of release dates. Re-verify the CoD axes against it before citing | `Cost of Delay ÷ Job Size`; CoD = business value + time criticality + risk/opportunity | Rank sibling roots when budget forces a sequence; "Job Size" ≈ a node's `estTokens`, so WSJF orders the ledger's phase-1 slice by value-per-token. |
...
- **Standard.** PMI *Practice Standard for Work Breakdown Structures* (current edition), codified in the **PMBOK Guide**. **The 8th Edition has shipped** — released Nov 2025 (ebook) with the print edition in Jan 2026 — and it restructures the guide again: 6 principles, 7 performance domains, 5 focus areas, and 40 non-prescriptive processes, with the PMP exam realigning to it on 9 July 2026. The WBS mapping below was drawn against the **7th Edition (2021)** and is **not yet re-verified against the 8th**; treat 7e as the stated baseline, say so when you cite it, and re-map before presenting a WBS argument as PMI-current. PMI also publishes continuous digital updates via PMIstandards+, so no print edition is ever the whole standard.
...
- **Pin, don't blend.** SAFe **6.0** and PMBOK **7th Edition** are the mapping baseline here — **and both now have successors** (AI-Native SAFe, 23 Jun 2026; PMBOK 8th Edition, Nov 2025 / Jan 2026 print). That does not make the mappings wrong; it makes them *dated*, which is a thing you state rather than hide. Update the WSJF axes and the WBS/CPM references **together** when you re-map — do not order one phase by SAFe 6 WSJF and the next by a newer definition.
- **Laws don't version, methods do.** Amdahl's and Brooks's Laws are mathematical/empirical results and need no edition tracking. WSJF, RICE inputs, MoSCoW, and the PMI standards **do** — re-check them on a yearly cadence, the same discipline `../../loop-review/references/owasp-cwe.md` applies to the OWASP/CWE editions. This file's own yearly cadence is what should have caught both successors above; it did not, which is why the confirmation log below records dates rather than promising diligence.
...
- **Confirmation log — 2026-07-26.** Verified against the primary source: **AI-Native SAFe released 23 June 2026** by Scaled Agile as a new version of the framework, with numbered versions being retired in favour of release dates (this pass removed an unqualified "SAFe 6.0 … current"); **PMBOK Guide 8th Edition** released Nov 2025 / Jan 2026 print, with the PMP exam realigning 9 July 2026 (this pass removed "re-check for a successor" — the successor exists). **Not independently re-confirmed and therefore left as named-without-new-precision:** the *Practice Standard for Work Breakdown Structures* edition number, RICE's canonical formulation, and the DSDM/AgilePF handbook edition behind MoSCoW — cite these by name and method, not by an edition number you have not read.
