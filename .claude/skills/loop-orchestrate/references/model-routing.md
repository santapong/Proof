...
The preconditions, the verbatim disclosure the skill must print before the planner spawns, and the automatic fallback — on a refusal, on an HTTP 400 from a zero-retention org, or on exceeding the declared latency budget, fall back to `claude-opus-5` at `max`, `log()` the fallback, and carry it into the cast ledger and the gate report — are specified in `../../loop-engine/references/execution-modes.md` §M7. Do not restate them here; read them there before using the flag.
