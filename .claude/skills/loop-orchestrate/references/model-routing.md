...
| Fable 5 | `claude-fable-5` | Opt-in only — `--planner fable` (the decompose node) or `--fable-gate` (one lens of an all-out gating vote) — eyes open to the caveats in the last section. |
...
- **Nothing here reaches Fable 5**, and the table cannot route to it. Fable is reachable only by the explicit opt-ins — `--planner fable` for task 3, `--fable-gate` for one lens of an all-out gating vote (§M7b).
...
## Fable 5 — opt-in only, via `--planner fable` or `--fable-gate`

Fable 5 (`claude-fable-5`) is reserved for the **single hardest narrow long-horizon reasoning task** in a project — a deep proof-style derivation, a gnarly multi-step planning problem, one the-whole-project-hinges-on-it analysis. It is never the default and never selected by the routing table on its own. **The two sanctioned entry points are the `--planner fable` flag**, which routes only the single decompose/planning node, **and the `--fable-gate` flag**, which routes exactly one lens of an all-out gating verify vote through a logged Opus fallback (§M7b). There is no other supported way to reach Fable 5 from this skill: no task type resolves to it, and no modifier promotes into it.
...
The bounding argument, stated plainly: the failure modes that make Fable dangerous in a fan-out are **silent** — a refused security-audit item reads as a clean bill of health. A refused or stalled **planner** fails **loudly**: one node, no DAG, nothing downstream runs, and a human is already standing at the gate. That asymmetry is the reason the planner is one of only two places an override is sanctioned — the other is a single lens of an all-out gating vote, where the per-lens fallback and the majority-refute reduce make the silent-refusal gap loud instead (§M7b). Both bound the override; neither repeals the rule, and it generalizes no further.
...
If a task tempts you toward Fable 5 but is *wide* (a fan-out) or *blocking without a human already at the gate*, that temptation is the signal to route it to `claude-opus-5` at `max` effort instead — the one exception being the §M7b gating lens, which is narrow inside the vote and carries its own logged fallback. Fable is a scalpel for one deep cut, not a tier you spread across a phase.
