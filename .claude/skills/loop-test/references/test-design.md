...
## 7. Test-smell catalogue: how a green suite lies

§3 names the six properties a good test holds; this section catalogues the named failures that ship anyway. Each smell is a test that lies in one of two directions — **green while the code is broken** (false confidence, the worse one) or **red while the code is fine** (the credibility drain §3.2 warns a suite cannot spare) — and both spend the same asset: what a green run is worth. Every one looks like a passing test until you probe it, so the detection signal comes before the fix.

| Smell | How it lies | Detection signal | Fix |
|---|---|---|---|
| **Assertion-free test** | Green while broken: it proves the unit ran without throwing and nothing else, so every wrong output passes — coverage-without-verification (§5) in its purest form. | The body asserts nothing about an outcome (or only `not.toThrow()`); an assertion-required lint rule flags it where the ecosystem ships one; under a mutation run (`standards.md`) every mutant on its covered lines survives. | State the contract the test exists to pin and assert it (§1). If no wrong output can be named, there is no behavior here — delete the test (§4's line). |
| **Mock-echo test** | Green while broken: the stub is configured to return X and the test asserts X came back. The value round-trips through the mocking library without touching your logic — the test verifies the mock. | The asserted value appears verbatim in the double's setup and the unit neither transforms, branches on, nor combines it. Confirm by gutting: replace the unit's body with a bare pass-through — an echo test stays green. | Assert what the unit *does* with the collaborator's answer — the transformation, the branch taken, the error propagated. If it genuinely does nothing to it, that is §4 pass-through code: no test. |
| **Change-detector test** | Red while fine: it asserts internal call sequences or private state, so every correct refactor fails it — and because it pins the implementation rather than the contract, an implementation that is faithfully wrong still passes. | It fails §3.1's refactor test; its assertions are spy call-counts on collaborators you own (§2's boundary rule broken); its history churns in step with the production file without ever having caught a defect. | Re-anchor on the observable outcome — §6's first pair is the rewrite. Keep interaction assertions only where the call *is* the contract (§2). |
| **Shared-fixture coupling** | Both directions: red while fine when reordering breaks an ordering assumption, and green while broken when state left by an earlier test masks the defect this one should catch. | Run the suite in random order (`pytest-randomly`, RSpec `--order random`, the runner's shuffle flag) and run failures alone — any result that depends on position is the finding (FIRST's *Independent*, `standards.md`). | Move module-level mutable fixtures into per-test setup and teardown (§3.4); a fixture too expensive to rebuild per test must be immutable — or a fake (§2). |
| **Sleep-based async test** | Red while fine on any machine slower than the guessed duration — flaky by construction — and every green run still pays the full sleep, so the suite slows to its own worst case (§3.3). | A literal duration in test code (`sleep(2)`, `setTimeout`-then-assert); failures that reproduce only on CI or under load and get "fixed" by raising the number. | Wait on the condition, never on time: await the actual promise or completion, or poll with the framework's `waitFor`/`eventually`, with a deadline as the failure bound — not as the wait. Time-driven *logic* takes an injected fake clock advanced explicitly (§2). |
| **Coverage-as-goal** | Green and certified while broken: once the number is the target, tests are written to make lines run, not to pin behavior — a coverage gate *manufactures* the assertion-free and mock-echo rows above. §5 owns the metric; this row is what enforcing it as a goal produces. | Coverage rises while assertion strength does not: a mutation run (`standards.md` → Stryker/PIT) shows mutants surviving under fully covered lines; new tests are named after functions rather than behaviors and land in threshold-chasing commits. | Keep coverage a floor on changed lines (§5) and audit strength with mutation testing. Rewrite or delete any test whose only purpose is moving the number. |
| **Testing the framework** | Green adds nothing — it re-proves what the dependency's own suite already proves (§4) — and red is noise: a minor upgrade of the dependency breaks your suite while your code is fine. | Delete all your production code from the traced path and the test still passes — nothing of yours is under test; the Arrange block is entirely framework configuration. | Delete it, or re-target the logic *you* added on top (§4). A framework boundary you genuinely distrust is a contract-testing problem (`standards.md` → Pact), not a unit test. |

Every row is one of this file's rules broken — §2's boundary discipline, §3's properties, §4's bar, §5's floor — with a name and a probe attached. The repair rule: fix per the table when a contract can be stated, delete when it cannot — §4's bar applies to existing tests as much as new ones. A smelly test kept "because it's coverage" is the exact trade this file's opening line forbids: it costs maintenance and pays false confidence.
