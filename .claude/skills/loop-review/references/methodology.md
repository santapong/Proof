...
Default to dropping. A review that reports three real, exploitable defects is worth more than one that reports thirty maybes — the maybes train the reader to ignore the tool. When in doubt between reporting and dropping, drop, and let the verification pass (`../../loop-engine/references/harness-policy.md` H4) be the place uncertain findings die, not the reader's inbox.

## The false-positive taxonomy — shapes that convince and are not findings

The false positives that survive this pipeline are not sloppy pattern matches — the Phase-3 trace kills those. They are candidates whose trace *looks* complete: the code is genuinely dangerous in isolation, the source→sink path reads as unbroken, and what is false is a premise the reviewer never checked. Each shape below names that premise and the cheapest check that settles it.

Ownership note: the suppression precedents themselves — test code, examples, vendored code, framework defaults, unreachable code, and every escape hatch — belong to `severity-model.md` (*False-positive exclusions*) and are not restated here; the rows point at them. What this table adds is the column that file does not carry: *why* each shape convinces a finder that has already done real work.

| Shape | Why it convinces | The kill-check |
|---|---|---|
| **Dead-code sink** — the vulnerable line is real; nothing reaches it | The line in isolation is a textbook sink misuse, so Phase 2 nominates it every time. The finder read the function and never asked who calls it. | Prove a live path in: grep every call site, then check the indirect routes named by the unreachable-code escape hatch in `severity-model.md`. No live entry point → Phase 3 reachability fails → no finding. |
| **Sanitizer-already-upstream** — input validated at a boundary the reviewer did not read | The function body genuinely lacks the check, and a 1-hop neighborhood can end below the middleware or schema layer where validation lives. Absence-in-view reads as absence. | Walk up to the actual entry point (the 2-hop escalation rule in *Sizing the neighborhood*) and read the boundary: route middleware, request schema, type coercion. A sanitizer on every path, correct for this sink, ends the trace — Phase 3 already says so. |
| **Framework-mitigates** — the framework supplies the missing control by default | The application code looks raw — no visible escaping, no visible token — precisely because the framework provides the control outside the diff. Reviewers review app code; defaults are invisible. | Open the sink's own wrapper (is `db.query` the raw driver or the ORM's parameterized helper?) and hunt the opt-out, per the framework-handled escape hatches in `severity-model.md`. Default on, no opt-out → drop. |
| **Config-not-code** — the "hardcoded secret" is a dev fixture | Keyword and entropy heuristics fire on the string itself; `PASSWORD = "hunter2"` reads as a leak regardless of what it unlocks. | Classify by blast radius (`standards.md`, *Secrets*): does the value authenticate against anything beyond the developer's own machine, and does the deploy path inject the real credential from env or a secret manager? A fixture that is also a live credential trips the test-code escape hatch and is a finding; a fixture that unlocks nothing is not. |
| **Wrong-trust-boundary** — the exploit requires the access it yields | The source→sink trace is genuinely unbroken; the threat model is what's wrong. "Injection" from an admin-only config file or a localhost-only debug port traces perfectly. | Name the privilege delta: what does the attacker hold after the exploit that they did not already need to launch it? Phase 1's rule applies — no crossed boundary, no vulnerability, only a bug. Zero delta → route to `quality-checks.md` if it is still a defect; otherwise drop. |

The middle column is the warning: every shape convinces by letting a correct local observation stand in for an unverified global one. The right column is the price of honesty — each kill-check is a grep, one file open, or one hop up the call chain. Cheap, and far cheaper than what shipping the false positive costs (`severity-model.md` names that anti-goal in its opening).

**A finding whose kill-check was not run cannot carry ≥ 0.8 confidence.** This is not a new scale: an unrun kill-check is exactly the "assumed step" that `severity-model.md`'s confidence table caps at 0.5–0.79, and the reporting bar does the rest. Every shape above tempts the reviewer to round up — the pattern looks so bad that the unchecked premise feels like a formality. It is not a formality; it is the entire question. Run the check or keep the cap.

## The asymmetry rule — down-rank, never silently delete

The two error costs are not symmetric. A false positive costs one review round: the author rebuts, the reviewer re-reads, the finding dies in daylight with a paper trail. A silenced true positive costs an incident, discovered on an attacker's schedule, with no paper trail at all. The taxonomy above must therefore *down-rank*, never make candidates vanish:

- **Kill-check ran and killed** → drop, and log *which check* killed it (`../../loop-engine/references/harness-policy.md` H6) so a human can catch an over-suppression — the same auditability `severity-model.md` demands of its exclusions, one field more specific.
- **Kill-check could not run** — the boundary file is outside the review surface, the deploy path is unknowable from the repo → the confidence cap above holds, and the candidate routes by `severity-model.md`'s reporting grid, which already sends a plausible Critical to the verify pass (H4) and never to the bin. The routing lives there; do not re-derive it here.
- **Never delete the record of the decision.** A suppression without a log is where silenced true positives are manufactured. The taxonomy makes dropping cheap and defensible; the log is what keeps it honest.

The asymmetry does not license reporting maybes — the bar stays where [The reporting bar](#the-reporting-bar) put it. It dictates what happens on the *far side* of the bar: below it, a candidate is logged or escalated, not erased.
