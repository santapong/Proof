---
name: loop-review
description: "Use when reviewing code before merging it — an authentication or login endpoint, a session or token flow, code handling user input, anything touching secrets or credential config, a payment or other sensitive feature, or a whole diff, PR or repository. Finds injection, broken authentication, crypto misuse, exposed secrets, SSRF and dependency risk, judged against OWASP Top 10, CWE Top 25 and ASVS. Reports HIGH and MEDIUM findings at confidence at or above 0.8 and changes nothing — it judges, it does not refactor. For applying the refactoring that fixes a reported smell, use loop-pattern. For a change-impact and release-risk report rather than a defect hunt, use loop-audit. For diagnosing a failure that was actually observed at runtime, use loop-debug. For a hypothesis about a system that has not failed — measuring whether a change helps rather than why something broke — use loop-experiment."
