...
This file has three parts: the four Diátaxis modes (§1–§5); the concrete artifacts you actually write and their required structure (§6–§10), with the decision table (§11) that maps a user need to the doc type to reach for; and the rot catalogue (§12) for diagnosing an existing doc set.
...
## 12. Rot catalogue: how each doc type dies

**Key rule: docs rot invisibly, because neither party in the transaction reports the failure — the writer never rereads what they shipped, and the reader assumes their confusion is their own fault.** Rotten code crashes; a rotten doc just quietly misdirects, and the misdirected reader files no bug. So rot must be *hunted*, not waited for. This section is the field guide for an existing doc set: each species, its detection signal, its fix, and the Diátaxis contract it breaks. How to write so rot cannot take hold is `style.md §7`; the verification law is SKILL §5 — this catalogue is what skipping them looks like six months later.

| Rot species | Detection signal | Fix | Violates |
|---|---|---|---|
| **The archaeological README** — describes the previous architecture | Names in the README (modules, dirs, commands) that no longer exist in the tree; a grep of README nouns against the repo is enough to convict | Rerun the quickstart and re-grep the nouns on every structural change — the quickstart is the canary that dies first | §6 — a front door that routes visitors to demolished rooms fails both halves of its job, orientation and routing |
| **The step-7 tutorial** — breaks partway; never re-executed since written | Last-edit date predates churn in every file the tutorial touches, and no verification stamp anywhere on the page | Runnable-or-dated (below): re-execute end to end on a clean environment, or stamp it | §2 — "it must work every time" is the tutorial's contract, and a mid-run failure is exactly where the beginner concludes *they* are the problem |
| **The mirror docstring** — `returns the user` above `-> User` | The delete test: remove the docstring; if the caller loses nothing, it was negative information | Delete it, or replace it with what the signature cannot say — units, side effects, invariants (§8) | §8 — reference that restates the lookup key answers no question anyone could arrive with |
| **The shrug changelog** — "fixes and improvements" | The entry answers none of the reader's three questions: can I upgrade, does it break me, do I care | Rewrite under §10's headings from the user's perspective; the SemVer bump must be derivable from the entries alone | §10 — information mode carrying no information |
| **The mode-mixed page** — tutorial drifts into reference dumps; how-to halts for theory | Headings switch verbs mid-page (*learn* → *look up*, *do* → *understand*); any smell test in §2–§5 fires | Split and cross-link; one document serving two modes serves neither reader | The cardinal rule (this file's intro) — never mix two modes in one document |
| **The unverified claim** — stated once, checked never | The claim cannot be traced to a source line you read or a command you ran | Verify at the source or delete; mark a genuine gap explicitly rather than papering over it (SKILL §5 — `style.md §2` prices the skipped check) | All four — accuracy is the floor every mode stands on |

**Runnable-or-dated.** A tutorial — and any runnable snippet in any mode — has exactly two honest states: re-executed end to end since the last change to what it exercises, or carrying an explicit "last verified against version X on date Y" stamp (the as-of stamping `style.md §7` demands of any time-bound fact, applied to executability). Unexecuted *and* undated is the third state: fiction with formatting. The stamp does not fix the doc; it converts silent rot into visible staleness, which the reader can price and the maintainer can triage. Silent rot can be priced by no one.

**Negative information.** The mirror docstring is worse than an empty one. An empty docstring costs nothing; the mirror spends the reader's attention and pays nothing back — and worse, it trains every reader that docstrings here are skippable, so the one docstring in the codebase that documents a real invariant goes unread. Deleting it is not laziness; it is refunding attention. §8's closing rule already draws the line — document what the signature cannot express — this entry is what ignoring that rule compounds into.
