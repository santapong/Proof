...
**On the name.** This file is deliberately *not* called `deployment.md`. Three files with that name already exist in this plugin and each means something different — `../../loop-design/references/deployment.md` is the design-time delivery-shape stub, `../../loop-autopilot/references/deployment.md` is about deploying *the loop itself*, and `../../loop-ship` owns actual deployment mechanics across five files. A fourth `deployment.md` meaning "the ops autonomy dial" would be the most confusing name available, and the naming symmetry is not worth it.
...
**The ladder has one definitional home: `../../loop-autopilot`.** OBSERVE → VERIFY → SUSTAIN → SCALE, the rung definitions, and the degradation guarantee that any alarm drops the loop one rung with a safe floor are defined there and are not restated here. This section says only what each rung *means when the object is a live service instead of a repository*.
...
6. **Tripwire.** Rollback rate over a window above threshold, a SUSTAIN drift alarm, or any runbook-success-rate collapse **drops autonomy one rung automatically and logs it**. Re-enabling requires a human. This is `loop-autopilot`'s degradation guarantee applied unchanged; the worst case is not a runaway actuator, it is a service that quietly goes back to paging a person.
...
**What makes it defensible anyway** is the same property that makes `loop-autopilot`'s ladder sound: it **degrades downward**. SCALE trips to VERIFY; VERIFY's floor — a human approves the risky ones — is always there to catch it; and every trip is recorded in a place a human is expected to read. That is not a claim that the automation is right. It is a claim that being wrong is cheap and visible, which is the only safety property this design actually earns.
