...
**The autonomy ladder has exactly one definitional home:** OBSERVE → VERIFY → SUSTAIN → SCALE, its rung definitions, and its degradation guarantee belong to `../loop-autopilot` (its `references/deployment.md` §Advanced, elaborating the README ladder table), and this skill cites those rungs for a live service without restating a word of them.
