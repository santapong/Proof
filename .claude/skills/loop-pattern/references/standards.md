...
**The plugin's three authority grades, as this shelf spends them.** Every standards shelf here uses the same three, with the same meanings — see `../../loop-integrate/references/standards.md` for the shelf where all three are in play. This file writes them as the `authoritative: true` / `false` flags in the tables below, and the mapping is exact:

- **Authoritative — yes** = `authoritative: true`. The body that owns the thing published the rule. On this shelf: **PEP 8**, **Rules of React**, the **Django** docs, and the **Spring Framework** reference — a framework's maintainers defining their own contract is the strongest form of "ratified and published" available for a framework.
- **Authoritative — draft** = *no entry on this shelf carries this grade.* The living documents below are **published and current**, not drafts; do not confuse "unversioned" with "unratified."
- **Authoritative — no** = `authoritative: false`. On this shelf: **GoF**, **Fowler**, **Bloch**, **SOLID**, the **Google style guides**, and the **Rust API Guidelines** (which explicitly call themselves guidelines rather than a mandate).
...
| **Spring Framework Reference Documentation — stereotype annotations** | VMware / Spring team (Broadcom) | **Spring Framework 7.0.x** — the current major, confirmed **2026-07-26**: 7.0 went GA on **13 Nov 2025** and the 7.0.x line is the current production branch (7.0.8, June 2026), with 7.1.x due November 2026. `docs.spring.io/spring-framework/docs/current` | **true** | `@Component` / `@Service` / `@Repository` / `@Controller` stereotyping, constructor injection over field injection, and bean-lifecycle idiom, applied when refactoring Spring bean wiring. |
...
- **Say "unconfirmed" rather than guessing.** If you cannot confirm a living document is still current, write *"unconfirmed as of \<date\>"* in the diff or the PR description instead of asserting it is. This shelf opens by warning that a fabricated ISBN or an invented edition makes a whole diff look careless; an unverified "confirmed live" claim is the same failure wearing a date.

**Confirmation log — 2026-07-26.** Verified against the primary source: **Spring Framework 7.0.x** as the current major (7.0 GA 13 Nov 2025; 7.0.x is the current production line as of mid-2026), which this pass restated with its GA date rather than a bare "as of July 2026". **Checked and unchanged:** the four unversioned living documents — **PEP 8** (`peps.python.org/pep-0008`), the **Google style guides** (`google.github.io/styleguide`), the **Rust API Guidelines** (`rust-lang.github.io/api-guidelines`), and **Rules of React** (`react.dev/reference/rules`) — remain live, published, and unversioned; the honest pin for each is the URL plus this date, and there is deliberately no number to quote. **Not independently re-confirmed and not needing it:** **GoF (1994)**, **Fowler 2e (Nov 2018, ISBN 978-0-13-475759-9)**, **Effective Java 3e (Jan 2018, ISBN 978-0-13-468599-1)**, and the **SOLID** essays are fixed bibliographic facts. **Deliberately unpinned:** the **Django** docs track whatever release the target project runs — confirm against *that* version's docs, never against a number cached here.
