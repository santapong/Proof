...
| Verifier width (§M5) | 1 verifier; 3 perspective-diverse when the ask is thorough/audit/comprehensive | Always diverse-lens: **3** for a standard node, **5** for a correctness-critical/gating node |
...
**Only two files parse flags:** `loop-engine/SKILL.md` step 1 and `loop-orchestrate/SKILL.md` step 1. The other sixteen skills advertise the flags in their `argument-hint` frontmatter and pass their **raw argument string** through when they invoke `loop-engine` — that pass-through *is* the inheritance mechanism, and it is precisely why no domain skill needs mode logic of its own.
...
**Lens *sets* are a decomposition decision, not a mode dial.** Mode picks *how many* of a node's declared lenses run; it never invents new ones. A node that declares three lenses and needs five under full mode needs a decomposition change — go and declare two more, deliberately, in the template.

**Dry threshold.** L1's K goes 2 → 3 in full mode:

```js
const DRY_LIMIT = input.mode === 'full' ? 3 : 2
```
...
width(n) = 1                               for a non-verify node
         ...
         = 5                               full mode, correctness-critical/gating
...
`BAND` — per-agent **output** tokens, optimize mode:

| Node kind | Band |
|---|---|
| scout / doc | 2k–6k |
| implement | 10k–25k |
| analyze / synthesize | 12k–30k |
| verify / judge / critic | 6k–18k |
| gating | 20k–45k |
| planner | 18k–45k |
| planner on Fable | 30k–80k |
...
> `--planner fable` routes the decompose node to claude-fable-5. Tradeoffs: markedly higher latency (a minutes-long turn on a gate-blocking node), a broader class of refusals than the rest of the fleet, and a 30-day data-retention requirement — an organization with zero data retention receives an HTTP 400 rather than a degraded result. On refusal, on a 400, or on exceeding the declared latency budget, this node falls back to claude-opus-5 at max effort and the fallback is logged.

**Automatic fallback.** On a refusal, on an **HTTP 400 from a zero-retention org**, or on exceeding the declared latency budget → fall back to `claude-opus-5` at `max`, `log()` the fallback, and carry it into the cast ledger and the gate report. The 400 is absorbed exactly as a refusal is; the design does not depend on being able to inspect the org's retention posture ahead of time, at the cost of one wasted round-trip in the ZDR case.

**Ledger requirement.** A run whose planner was Fable **says so** in its cast row and in its gate deliverable. A reader must never have to guess which model produced the DAG they are being asked to approve.
...
A template that does not use loops omits `DRY_LIMIT`; one with no verify stage omits `WIDTH`. `--planner fable` overrides `ROUTES[MODE].planner.model` when `input.planner === 'fable'`. **No other local variation is permitted.**
...
3. **Eighteen skills, no per-skill logic.** Domain skills advertise the flags in `argument-hint`, pass their argument string through to `loop-engine`, and author through these templates. A skill that grows its own mode branch is a defect, not a feature. The one shared edit every skill does take is pasting the identical `ROUTES` block into any template that sets `model` or `effort` — boilerplate from a single source of truth, not per-skill behavior.
