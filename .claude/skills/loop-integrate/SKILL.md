...
argument-hint: <integration> [--mode <lite|balanced|all-out>]
---
...
**Execution flags.** `--mode <lite|balanced|all-out>` is advertised here and **parsed by `loop-engine`, never by this skill** — pass the raw argument string through when you invoke it. See `../loop-engine/references/execution-modes.md`.
